...
int ObjReader::init(std::string file, float scaleCo)
{
...
int ObjReader::update(std::vector<glm::vec3> newVertices)
{
	m_vertices = newVertices;
	if (computeFaceNormals(true) != 0)
		return -2;
...
int ObjReader::computeVertexColors()
{
	if (m_vertices.empty())
		return 0;
...
int ObjReader::initBuffer()
{
	...
	QOpenGLContext* fz = QOpenGLContext::currentContext();
	QOpenGLFunctions* f = fz->functions();
...
int ObjReader::scale(float scale)
{
...
int ObjReader::loadObj(std::string file)
{
...
int ObjReader::writeFile(std::string objFile)
{
...
int ObjReader::computeFaceNormals(bool recompute)
{
	...
	for (int i = 0; i < degeneratedTris.size(); ++i)
	{
		...
		OutputDebugStringW(L"Removing degenerated triangles");
		m_triangles.erase(m_triangles.begin() + degeneratedTris[i] - i);
	}

	return 0;
}
int ObjReader::computeVertNormals()
{
	computeFaceNormals();
	m_vertNormals.clear();
	for (int i = 0; i < m_vertices.size(); ++i)
	{
		...
		float total = std::accumulate(areas.begin(), areas.end(), static_cast<float>(0.0));

		for (int j = 0; j < triangles.size(); ++j)
		{
			if (!isFinite(m_faceNormals[triangles[j]].x)
				|| !isFinite(m_faceNormals[triangles[j]].y)
				|| !isFinite(m_faceNormals[triangles[j]].z))
			{
				//LOG_ERROR_MSG("Found invalid face normals");
				return -1;
			}

			if (!isFinite(areas[j]) || !isFinite(total))
			{
